...
### Aug 18 ###
# I THINK COMPARING WORKS NOW JUST BE CAREFUL LABELING
# MAJOR: PIXEL SPACING/DISTANCE DIFFERENCE CALCULATION LIKELY INCORRECT
# Can bulk name rows, but need to close window to manually edit it
...
class DICOMViewer:
    ...
    def compare_measurements(self):
        ...
        # Load DICOM from the same folder as file1
        state_folder1 = os.path.dirname(file1)
        dicom_filename1 = data1.get("dicom_filename", None)

        if dicom_filename1:
            dicom_path1 = os.path.join(state_folder1, dicom_filename1)
            if os.path.exists(dicom_path1):
                dicom1 = pydicom.dcmread(dicom_path1)
                pixel_data1 = dicom1.pixel_array
            else:
                messagebox.showerror("Error", f"Cannot find DICOM file next to working file:\n{dicom_path1}")
                return
        else:
            messagebox.showerror("Error", "No DICOM filename stored in file1")
            return

        # Since both working files come from the same DICOM, reuse pixel_data1
        pixel_data2 = pixel_data1
        ...
        def write_row(joint, f1_idx, f2_idx):
            ...
            dists = [euclidean_dist(dx, dy) for dx, dy in diffs]
    ...
    def label_window(self):
        ...
        label_button = tk.Button(win, text="Label Starting At", command=label_starting_at)
