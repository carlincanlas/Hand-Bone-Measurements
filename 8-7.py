...
class DICOMViewer:
    def __init__(self, root):
        ...
        self.wc_slider = tk.Scale(wc_frame, from_=0, to=255, orient=tk.HORIZONTAL, command=self.update_window_center)
        #Window Width (Contrast)
        ww_frame = tk.Frame(self.control_frame)
        tk.Label(ww_frame, text="Contrast:").pack(side=tk.LEFT)
        self.ww_slider = tk.Scale(ww_frame, from_=1, to=512, orient=tk.HORIZONTAL, command=self.update_window_width)
        ...
        tk.Button(export_frame, text="Export", command=self.export_measurements).pack(side=tk.LEFT)
        self.save_images_var = tk.BooleanVar()
        self.save_images_checkbox = tk.Checkbutton(export_frame, text="Include images", variable=self.save_images_var).pack(side=tk.LEFT)
        ...
        self.slider = tk.Scale(self.control_frame, from_=1, to=1, orient=tk.HORIZONTAL, command=self.slider_moved, showvalue=False)
        ...
        tk.Button(nav_frame, text="<", command=self.prev_frame).pack(side=tk.LEFT)
        tk.Button(nav_frame, text=">", command=self.next_frame).pack(side=tk.LEFT)
        ...
        jump_frame = tk.Frame(self.control_frame)
        jump_frame.pack(pady=5, fill='x')
        tk.Label(jump_frame, text="Jump to frame:").pack(side=tk.LEFT)
        ...
        self.wc_slider.pack(side=tk.RIGHT, padx=5, fill='x', expand=True) 
        wc_frame.pack(pady=5, fill='x')
        # Width (Contrast)
        self.ww_slider.set(self.window_width)
        self.ww_slider.pack(side=tk.RIGHT, padx=5, fill='x', expand=True)
            ...
            if key in frame_measures:
                ...
                if bone:
                    ...
                    if dx == dy == 0:
                        ...
                        offset_amount = 5  # You can tweak this value
    ...
    def on_mouse_press(self, event):
        ...
        if self.measure_step == 'bone_start':
            ...
            p1 = (x, y)                                                         # Store final mouse click for H's p1
        ...
        def dist_sq(a, b):                                              # Get squared distance between 2 points
    ...
    def export_measurements(self):
        ...
        ws.append(["Filename", "Frame", "h (mm)", "H (mm)", "OR (%)", "Bone Slope"])
        ...
        try:
            ...
            # Check if "save images" checkbox is checked
            if self.save_images_var.get():
                # Save images to a sibling folder based on Excel file path
    ...
    def save_working_file(self):
        ...
        dicom_name = os.path.splitext(os.path.basename(self.dicom.filename))[0] if self.dicom and hasattr(self.dicom, 'filename') else "DICOM"
        ...
        try:
            data = {
                "dicom_path": self.dicom.filename,
    ...
    def load_working_file(self):
        ...
        try:
            ...
            if not os.path.exists(dicom_path):
                messagebox.showerror("Missing DICOM", f"Original DICOM file not found:\n{dicom_path}")
                self.focus_app_window()
                return

            self.dicom = pydicom.dcmread(dicom_path)
            self.pixel_data = self.dicom.pixel_array
            self.pixel_spacing = [float(sp) for sp in self.dicom.PixelSpacing] if hasattr(self.dicom, 'PixelSpacing') else [1.0, 1.0]
            self.num_frames = self.pixel_data.shape[0] if self.pixel_data.ndim == 3 else 1
            ...
            self.slider.config(to=self.num_frames, state='normal')
            self.filename_label.config(text=f"File: {os.path.basename(dicom_path)}")
            self.show_frame()
