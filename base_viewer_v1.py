...
# OLD VERSION (Control buttons along the top, limited features, old measurement method)
# Export measurements
...
class DICOMViewer:
    def __init__(self, root):
        ...
        self.filename_label = tk.Label(frame_1, text="No file loaded")
        self.filename_label.pack(side=tk.LEFT, padx=(0, 10))

        tk.Button(frame_1, text="Load DICOM File", command=self.load_file).pack(side=tk.LEFT)

        # Add measurement button
        tk.Button(frame_1, text="Measure", command=self.start_measurement_workflow).pack(side=tk.LEFT, pady=2)
        tk.Button(frame_1, text="Clear Measurements", command=self.clear_measurements).pack(side=tk.LEFT, pady=2)
        tk.Button(frame_1, text="Export Measurements", command=self.export_measurements).pack(side=tk.LEFT, pady=2)
    ...
    def show_frame(self):
        ...
        # Apply zoom by adjusting axes limits
        img_height, img_width = frame.shape
        ...
        x0 = center_x - zoom_width / 2
        x1 = center_x + zoom_width / 2
        y0 = center_y + zoom_height / 2
        y1 = center_y - zoom_height / 2

        if x0 != x1:
            self.ax.set_xlim(x0, x1)
        else:
            self.ax.set_xlim(x0 - 1, x1 + 1)

        if y0 != y1:
            self.ax.set_ylim(y0, y1)
        else:
            self.ax.set_ylim(y0 - 1, y1 + 1)
        ...
        # Finalized h and H measurements
        ...
        # Temporary points and line during bone line
        if len(self.points) == 1:
            self.ax.plot(self.points[0][0], self.points[0][1], marker=dot, markersize=dotsize, color=colour)
        elif len(self.points) == 2:
            # Show bone line preview before confirmation
            p1, p2 = self.points
            self.ax.plot([p1[0], p2[0]], [p1[1], p2[1]], linestyle='dashed', linewidth=linesize, color=colour)
            self.ax.plot(p1[0], p1[1], marker=dot, markersize=dotsize, color=colour)
            self.ax.plot(p2[0], p2[1], marker=dot, markersize=dotsize, color=colour)
    ...
    def ask_bone_line_confirmation(self):
        ...
        self.label.config(text="Step 3: Click end of epiphysis")
        self.points.clear()
    ...
    def on_mouse_move(self, event):
        ...
        if part == 'p1':
            ...
            # Sync x2 of the other measurement
            ...
            if other_key in self.measurements[self.frame_index]:
                op1, op2 = self.measurements[self.frame_index][other_key]
                # Calculate new y2 for the other measurement using its slope
                ody = (x - op1[0]) * slope
                synced_op2 = (x, op1[1] + ody)
                self.measurements[self.frame_index][other_key] = (op1, synced_op2)
    ...
    def on_left_key(self, event): self.prev_frame()
    def on_right_key(self, event): self.next_frame()
    def on_key_press(self, event):
        if event.key in ('Delete', 'BackSpace'):
            self.delete_measurement()

    def delete_measurement(self, event=None):
        pass
