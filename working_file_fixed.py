...
### Aug 13 ###
# Something is wrong with comparing h and H measurements (click 1,2,3)
...
class DICOMViewer:
    def __init__(self, root):
        ...
        tk.Button(file_frame, text="Load File", command=self.load_file).pack(side=tk.RIGHT)   # Load file button
        ...
        tk.Button(summary_frame, text="Frames Measured", command=self.show_frames_measured).pack(fill="x")
    ...
    def on_mouse_press(self, event):
        ...
        if self.measure_step == 'bone_start':
            ...
            self.measurements[self.frame_index]['h_raw'] = (p1, p2)             # Store point where user clicked
            '''  measurements = { 0: {   # frame index 0
                                            'h': ((proj_x1, proj_y1), (proj_x2, proj_y2)),     
                                            'h_raw': ((click_x1, click_y1), (click_x2, click_y2)),
                                            'H': ((proj_x1_H, proj_y1_H), (proj_x2_H, proj_y2_H)), 
                                            'H_raw': ((click_x1_H, click_y1_H), (click_x2_H, click_y2_H)) }, ... '''
            ...
            self.measurements[self.frame_index]['H'] = (proj1, proj2)           # Store H measurements in current frame's measurements dict
            self.measurements[self.frame_index]['H_raw'] = (p1, p2)
            self.measure_step = None                                            # self.measurements == { 'h': (p1, p2),  'H': (p1, p2) }
            self.show_frame()
            return
    ...
    def compare_measurements(self):
        ...
        for joint in all_joints:
            ...
            for i in range(n_compare):
                ...
                h1 = self.calc_dist_from_points(f1.get('h')) if 'h' in f1 else None
                h2 = self.calc_dist_from_points(f2.get('h')) if 'h' in f2 else None
                H1 = self.calc_dist_from_points(f1.get('H')) if 'H' in f1 else None
                H2 = self.calc_dist_from_points(f2.get('H')) if 'H' in f2 else None

                or1 = (h1/H1*100) if h1 is not None and H1 not in (None,0) else None
                or2 = (h2/H2*100) if h2 is not None and H2 not in (None,0) else None

                h_pt1_dx, h_pt1_dy = coord_diff_mm(
                    f1.get('h_raw')[0] if 'h_raw' in f1 else None,
                    f2.get('h_raw')[0] if 'h_raw' in f2 else None)
                
                h_pt2_dx, h_pt2_dy = coord_diff_mm(
                    f1.get('h_raw')[1] if 'h_raw' in f1 else None,
                    f2.get('h_raw')[1] if 'h_raw' in f2 else None)
                ...
                write_number(ws, row_idx, 4, diff_val(h1, h2))
                write_number(ws, row_idx, 5, diff_val(H1, H2))
                write_number(ws, row_idx, 6, diff_val(or1, or2))

                write_number(ws, row_idx, 7, h_pt1_dx)
                write_number(ws, row_idx, 8, h_pt1_dy)
                write_number(ws, row_idx, 9, h_pt2_dx)
                write_number(ws, row_idx, 10, h_pt2_dy)
                write_number(ws, row_idx, 11, H_pt1_dx)
                write_number(ws, row_idx, 12, H_pt1_dy)
                write_number(ws, row_idx, 13, H_pt2_dx)
                write_number(ws, row_idx, 14, H_pt2_dy)
    ...
    def export_measurements(self):
        ...
        # Updated header (no Bone Slope)
        ws.append([
            "Filename", "Frame", "h (mm)", "H (mm)", "OR (%)",
            "h pt1 (x_mm,y_mm)", "h pt2 (x_mm,y_mm)",
            "H pt1 (x_mm,y_mm)", "H pt2 (x_mm,y_mm)"
        ])
        ...
        for frame_num in sorted(self.measurements.keys()):
            ...
            h_dist = None
            H_dist = None
            h_pt1_str = h_pt2_str = H_pt1_str = H_pt2_str = ""

            if 'h' in frame_data:
                h_pt1, h_pt2 = frame_data['h']
                h_dist = round(self.calculate_distance(h_pt1, h_pt2), 2)

                # Convert pixel coords to mm
                h_pt1_mm = (h_pt1[0] * col_spacing, h_pt1[1] * row_spacing)
                h_pt2_mm = (h_pt2[0] * col_spacing, h_pt2[1] * row_spacing)
                h_pt1_str = f"({h_pt1_mm[0]:.2f}, {h_pt1_mm[1]:.2f})"
                h_pt2_str = f"({h_pt2_mm[0]:.2f}, {h_pt2_mm[1]:.2f})"
    ...
    def show_frames_measured(self):
        ...
        if hasattr(self, "current_working_file") and self.current_working_file:
            base_name = os.path.basename(self.current_working_file)
        elif hasattr(self.dicom, "filename") and self.dicom.filename:
            base_name = os.path.basename(self.dicom.filename)
        else:
            base_name = "Frames"

        win = tk.Toplevel(self.root)
        win.title(f"{base_name} Frames Measured")  # <-- dynamic title
